...
func (knn *KNNClassifierStruct) predict(row []float64, nNeighborhood int) string {
	...
	for i := 0; i < n_rows; i++ {
		row_dense := utils.GetRowData(knn.X, i)
		distance := utils.EuclideanDistance(row_dense, row)
